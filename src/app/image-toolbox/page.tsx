	...
	const blob = await new Promise<Blob>((resolve, reject) => {
		canvas.toBlob(
			result => {
				...
				else reject(new Error('Unable to generate WEBP file'))
			},
			'image/webp',
			quality
		)
	...
	const handleConvertImage = useCallback(
		async (index: number) => {
			...
			try {
				...
				console.error(error)
				alert('Something went wrong during conversion. Please try again later.')
		...
		<div className='relative px-6 pt-32 pb-12 text-sm max-sm:pt-28'>
			<div className='mx-auto flex max-w-3xl flex-col gap-6'>
				<motion.div
					...
					<p className='text-secondary'>Select images → adjust quality → convert and download</p>
				</motion.div>
				...
				<motion.label
					...
					<div>
						...
						<p className='text-secondary text-xs'>Supports PNG, JPG, JPEG, HEIC, and other common formats</p>
					</div>
				</motion.label>
					...
					<div className='flex flex-wrap items-center gap-4'>
						<div className='flex-1 space-y-4'>
							<div>
								...
								<p className='text-xs text-slate-500'>Uses canvas.toDataURL('image/webp', {quality.toFixed(2)})</p>
							</div>
