	...
	return (
		<DialogModal open onClose={handleClose} className='card w-md max-sm:w-full'>
			<div className='space-y-4'>
				...
				<div>
					<label className='text-secondary mb-2 block text-sm font-medium'>Select images (multiple allowed)</label>
					...
					{images.length === 0 ? (
						...
						<>
							<div className='relative flex h-40 items-center justify-center overflow-visible rounded-xl bg-linear-to-br from-gray-50 to-gray-100'>
								...
								{images.length > 3 && (
									<div className='absolute right-4 -bottom-2 rounded-full bg-black/70 px-3 py-1 text-xs text-white shadow-lg'>{images.length} total</div>
								)}
							</div>

							<div className='mt-3 flex items-center justify-between'>
								<span className='text-secondary text-xs'>{images.length} images selected</span>
								<button
									type='button'
									onClick={() => fileInputRef.current?.click()}
				...
				<div>
					<label className='text-secondary mb-2 block text-sm font-medium'>Description (optional; applies to all selected images)</label>
					<textarea
						value={description}
						onChange={e => setDescription(e.target.value)}
						placeholder='Description for this image group...'
