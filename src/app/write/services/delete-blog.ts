...
export async function deleteBlog(slug: string): Promise<void> {
	...
	toast.info('Collecting article files...')
	...
	const commitData = await createCommit(token, GITHUB_CONFIG.OWNER, GITHUB_CONFIG.REPO, `Delete article: ${slug}`, treeData.sha, [latestCommitSha])

	toast.info('Updating branch...')
	await updateRef(token, GITHUB_CONFIG.OWNER, GITHUB_CONFIG.REPO, `heads/${GITHUB_CONFIG.BRANCH}`, commitData.sha)

	toast.success('Deleted successfully. Wait for deployment, then refresh.')
}
